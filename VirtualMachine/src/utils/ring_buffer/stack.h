...
typedef struct {
  ...
  stackElementT aContents[SIZE_COMBUF_B];
  int maxSize;
  int top;
} stackT;
...
/*
 * Functions: StackPush, StackPop, StackGetTop
 * Usage: StackPush(&stack, element); element = StackPop(&stack); element = StackGetTop(&stack);
 * --------------------------------------------
